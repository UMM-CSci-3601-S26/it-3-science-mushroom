...
public class AuthService {
  // Set a default job role for volunteers who don't have one assigned yet, so they still get basic permissions.
  ...
  /**
   * login checks the provided username and password, and if valid, creates and returns an
   * authenticated session with a JWT token and access profile.
  ...
  /**
   * signup creates a new user account with the provided information, and if successful, creates and
   * returns an authenticated session for the new user.
  ...
  /**
   * getCurrentAccessProfile takes a JWT token string, validates it, and returns the access profile for
   ...
   * @throws UnauthorizedResponse if the token is missing, invalid, expired, or if the corresponding user
   *                              account no longer exists or has no system role
   */
  ...
  /**
   * buildAccessProfile takes a system role and an optional job role, and returns a
   * map representing the access profile for the user.
  ...
  /**
   * buildAccessProfile overload that takes a Users object and extracts the system role
   * and job role to build the access profile.
   ...
   * @throws BadRequestResponse if the user is not found, if the user is
   *                            not a volunteer, or if the job role does not exist
   */
  ...
  /**
   * createSession creates an AuthSession for a given user and system role, which includes
   * generating a JWT token and building the access profile.
   * @param user the Users object representing the authenticated user
   * @param systemRole the system role of the user
  ...
  /**
   * normalizeAccessIdentity takes a Users object and ensures it has a valid system role and job role (if applicable),
   * and returns an AccessIdentity object that represents the user's identity for permission checks.
   * This method also performs lazy repair of old volunteer accounts that may not have a job role
   * assigned yet by assigning them the default volunteer job role.
   * @param user the Users object representing the authenticated user
   * @return an AccessIdentity object containing the normalized identity information for permission checks
   * @throws UnauthorizedResponse if the user account has no system role or if a volunteer
   *                              account has no job role and can't be repaired
   */
  ...
  // AccessIdentity is a simple value class to hold the relevant identity information extracted
  // from the Users object for permission checks.
