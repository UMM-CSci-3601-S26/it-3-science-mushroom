...
export class InventoryService {
  ...
  resetAllQuantities(): Observable<unknown> {
    return new Observable(observer => {
      this.httpClient.patch(`${this.inventoryUrl}/resetQuantity`, {}).subscribe({
        next: (result) => {
          //this.loadInventory();
          observer.next(result);
          observer.complete();
  ...
  resetMatchingQuantities(filters: {item?: string; brand?: string; color?: string; size?: string; type?: string; material?: string}): Observable<InventoryBulkActionResponse> {
    ...
    return this.httpClient.patch<InventoryBulkActionResponse>(`${this.inventoryUrl}/resetQuantity`, {}, { params: httpParams });
  }
